...
function AllTickets() {
  ...
  const [currentPage, setCurrentPage] = useState(1); // Start from 1 for Pagination
  ...
  useEffect(() => {
    fetch("https://crm.bees.in/api/v1/status/tickets/all")
      ...
      .then(data => {
        ...
        // Find the "Open" status and set it as the default selected status
        const openStatus = reversedData.find(
  ...
  const handleStatusChange = event => {
    ...
    if (statusId === "all") {
      // Fetch all tickets
      fetch("https://crm.bees.in/api/v1/ticket/all")
        .then(response => response.json())
        .then(data => {
          const reversedData = Array.isArray(data) ? data.reverse() : [];
          setTickets(reversedData); // Now tickets are reversed!
          setLoading(false);
        })
        .catch(error => {
      ...
      fetchTickets(statusId);
    }
  };

  const fetchTickets = statusId => {
    console.log(`https://crm.bees.in/api/v1/ticket/status/${statusId}`);
    fetch(`https://crm.bees.in/api/v1/ticket/status/${statusId}`)
      .then(response => response.json())
      .then(data => {
  ...
  const handleAssign = status => {
    ...
    //     status: "66a4b88dbfb91567d9e78627",
    ...
    try {
      const response = axios.put(
        `https://crm.bees.in/api/v1/ticket/${selectedTicket._id}`,
        {
          assignToEmployeeId: selectedEmployeeId,
          status: openStatusId,
          startTime: new Date().toISOString(),
      ...
      const openStatus = statuses.find(status => status.status === "Pending");
      if (openStatus) {
        setSelectedStatusId(openStatus._id);
        fetchTickets(openStatus._id);
      }
  ...
  const handleSave = async selectedTicket => {
    const currentTime = new Date().toISOString();
    setLoading(true);

    try {
      // Determine if endDate should be included based on the status
      const shouldIncludeEndDate = statuses.some(
        status =>
          status._id === selectedModalStatusId && status.status === "Closed"
      );
      ...
      if (shouldIncludeEndDate) {
        payload.endDate = currentTime;
      }
      ...
      // Fetch tickets with the status "Open"
      const openStatus = statuses.find(status => status.status === "Open");
      if (openStatus) {
        setSelectedStatusId(openStatus._id);
        fetchTickets(openStatus._id);
  ...
  const statusOptions = modalStatuses
  ...
  const handleDelete = async ticketId => {
    try {
      console.log(`https://crm.bees.in/api/v1/ticket/delete/${ticketId}`);
      await axios.delete(
        `https://crm.bees.in/api/v1/ticket/delete/${ticketId}`
      );
              ...
              {displayTickets.map(ticket => (
                <>
                  <tr key={ticket._id}>
                    ...
                    {/* <td className="py-2 px-4 border-b text-center">
        {getStatusNameById(ticket.status)}
      </td> */}
                    {selectedStatusId === "all" && (
                      <td className="py-2 px-4 border-b text-center">
                    ...
                    <td className="py-2 px-4 border-b text-center">
                      ...
                      {!isClosedStatus(ticket.status) && (
                        <button
                          ...
                          className="mr-1 px-2 py-1 bg-green-500 text-white rounded-md"
                          style={{ backgroundColor: "#4b57ad" }}
                        >
                          Edit
                        </button>
      ...
      {isCloseModalOpen && (
        ...
        >
          <div className="p-4">
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              >
                ...
                <textarea
                  ...
                  name="comment"
                  value={comment} // Set textarea value from state
                  onChange={handleCommentChange} // Handle change to update state
                  style={{
                    width: "100%",
                    padding: "10px",
            ...
            <button
              onClick={() => handleSave(selectedTicket)}
