...
function EmployeeOpenTicket() {
  ...
  useEffect(() => {
    fetch("https://crm.bees.in/api/v1/status/tickets/all")
      ...
      .then(data => {
        setStatuses(data);
        const openStatus = data.find(status => status.status === "Open");
        if (openStatus) {
  ...
  const handleStatusChange = event => {
    ...
    if (statusId === "all") {
      ...
      fetch(`https://crm.bees.in/api/v1/ticket/all/employee/${userId}`)
        .then(response => response.json())
        .then(data => {
  ...
  const fetchTickets = statusId => {
    ...
    const userId = JSON.parse(localStorage.getItem("user")).userId;
    console.log(
      `https://crm.bees.in/api/v1/ticket/status/${statusId}/${userId}`
    );

    fetch(`https://crm.bees.in/api/v1/ticket/status/${statusId}/${userId}`)
      .then(response => response.json())
      .then(data => {
  ...
  // console.log(selectedTicket?.statusComment)
      ...
      {selectedTicket && isEditModalOpen && (
        ...
        >
          <div className="p-4">
            ...
            <button
              onClick={() => {
                handleAssign(selectedTicket?.statusComment);
              }}
...
export default EmployeeOpenTicket;
