...
function ReportByEmployee() {
  ...
  useEffect(() => {
    axios
      .get("https://crm.bees.in/api/v1/employee")
      .then(response => {
        const sortedEmployees = response.data.sort((a, b) =>
          a.name.localeCompare(b.name)
        ); // Sort alphabetically by name
        setTicketTypes(sortedEmployees);
      })
      .catch(error => {
  ...
  const handleFetchData = () => {
    ...
    const url =
      option === "all"
        ? `https://crm.bees.in/api/v1/reports/closed/employee?${
            selectedAllCustomers ? "" : `employeeIds=${ticketTypeIds}`
          }${selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`}`
        : `https://crm.bees.in/api/v1/reports/date/employee?${
            selectedAllCustomers ? "" : `employeeIds=${ticketTypeIds}`
          }${
            selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`
  ...
  const handleDownload = () => {
    ...
    const url =
      option === "all"
        ? `https://crm.bees.in/api/v1/reports/csv/employee?${
            selectedAllCustomers ? "" : `employeeIds=${ticketTypeIds}`
          }${selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`}`
        : `https://crm.bees.in/api/v1/reports/csv/employee?${
            selectedAllCustomers ? "" : `employeeIds=${ticketTypeIds}`
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-4">
      ...
      <div className="mt-6">
        <label className="mr-4">Select Employee Name:</label>
        <Select
