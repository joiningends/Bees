...
function ReportByCustomer() {
  ...
  const [selectedTicketTypes, setSelectedTicketTypes] = useState([]); // Changed to an array
  ...
  const [selectedTicketStatuses, setSelectedTicketStatuses] = useState([]); // Changed to an array
  ...
  useEffect(() => {
    axios
      ...
      .then(response => {
        ...
        setTicketTypes(sortedCustomers);
      })
      .catch(error => {
        toast.error("Failed to fetch Customer Names");
        console.error("There was an error fetching the Customer Names!", error);
      });

    axios
      .get("https://crm.bees.in/api/v1/status/tickets/all")
      .then(response => {
  ...
  const handleSelectAllStatusChange = e => {
    ...
    if (isChecked) {
      ...
      setSelectedTicketStatuses([]); // Clear selected ticket statuses
    } else {
      setSelectedStatus("");
      setSelectedTicketStatuses([]); // Reset ticket statuses dropdown
    }
  };
  ...
  const handleFetchData = () => {
    ...
    const url =
      option === "all"
        ? `https://crm.bees.in/api/v1/reports/closed/customer?${
            selectedAllCustomers ? "" : `customerIds=${ticketTypeIds}`
          }${selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`}`
        : `https://crm.bees.in/api/v1/reports/date/customer?${
            selectedAllCustomers ? "" : `customerIds=${ticketTypeIds}`
          }${
            selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`
  ...
  const handleDownload = () => {
    ...
    const url =
      option === "all"
        ? `https://crm.bees.in/api/v1/reports/csv/customer?${
            selectedAllCustomers ? "" : `customerIds=${ticketTypeIds}`
          }${selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`}`
        : `https://crm.bees.in/api/v1/reports/csv/date/customer?${
            selectedAllCustomers ? "" : `customerIds=${ticketTypeIds}`
          }${
            selectedStatus === "all" ? "" : `&statusIds=${ticketStatusIds}`
          }&fromDate=${fromDate}&toDate=${toDate}`;
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-4">
      ...
      <div className="mt-6">
        <label className="mr-4">Select Customer Name:</label>
        <Select
