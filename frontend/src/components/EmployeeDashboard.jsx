...
function CustomerDashboard() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://crm.bees.in/api/v1/users/dash/bord/employee/${userId}`)
      .then(response => {
  ...
  // Fixed icons for specific statuses
  const iconList = {
  ...
  // Dynamic icons for up to 10 unique statuses
  const dynamicIcons = [
    PersonIcon,
    GroupIcon,
    WarningIcon,
  ...
  // Get an icon for each status
  const getIcon = (status, index) => {
    if (iconList[status]) {
      return iconList[status];
    }
    return dynamicIcons[index % dynamicIcons.length] || HelpIcon; // Fallback icon
  };
  ...
  return (
    ...
    >
      <button
        onClick={() => navigate("/Employee/Ticket")}
        style={{
          marginBottom: "20px",
          padding: "10px 20px",
        ...
        >
          {ticketStatuses.length > 0 &&
            ticketStatuses
              ...
              .map((ticket, index) => {
                const Icon = getIcon(ticket.status, index);
                return (
                  <div
                    key={index}
                    style={{
